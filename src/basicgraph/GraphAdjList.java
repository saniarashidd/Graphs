...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 List<Integer> outNeighbors = adjListsMap.get(v);
		 List<Integer> secondHops = new ArrayList<Integer>();
		 for(int i = 0; i < outNeighbors.size(); i++) {
			List<Integer> neighbors = getNeighbors(outNeighbors.get(i));
			for(int j = 0; j < neighbors.size(); j++) {
				secondHops.add(neighbors.get(i));
			}
		 }
		 return secondHops;
